...
export default function ChatInfo({ name, email, location, birthday, onNotesClick }: ChatInfoProps) {
    return (
            ...
            <CardContent className="flex flex-col items-center justify-between h-full p-6">
                ...
                <div className="flex flex-col items-center mb-6 text-center">
                    <div className="w-20 h-20 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center">
                        <span className="text-4xl font-semibold text-white">{name.charAt(0)}</span>
                    </div>
                ...
                <div className="space-y-2 w-full">
                    {renderButton(<FileText className="w-4 h-4 text-gray-700" />, "Notes", onNotesClick)}
                </div>
            </CardContent>
        </Card>
    );
...
const renderButton = (icon: JSX.Element, label: string, onClick: () => void) => (
    <Button
        variant="outline"
